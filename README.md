...
На уровне первой нормальной формы были устранены повторяющиеся группы атрибутов, а значения всех полей сделаны атомарными. Например, в таблицах app_user и vehicle каждая запись описывает одного пользователя или одно транспортное средство соответственно, без хранения множественных значений в одном поле.
На уровне второй нормальной формы все неключевые атрибуты полностью зависят от первичного ключа. В большинстве таблиц используется простой ключ — поле id, поэтому зависимость очевидна: атрибуты name, price, model и т.п. в таблице vehicle определяются конкретным транспортным средством, а поля city, email, role в таблице app_user – конкретным пользователем.
На уровне третьей нормальной формы исключены транзитивные зависимости между неключевыми атрибутами. Все неключевые атрибуты зависят только от первичного ключа и не зависят друг от друга. Например, в таблице ordering поля date, address_start, address_end, price и status зависят только от id заказа, а не от данных о пользователе или автомобиле – эти сведения вынесены в отдельные таблицы (app_user и vehicle), с которыми установлены внешние связи. 
